...
export const generateAuthUrl = async (
  ...
  try {
    ...
    const data = result.data as any;
    console.log("getConnectUrl responses:", JSON.stringify(data, null, 2));
...
export const syncAccounts = async (
  ...
  try {
    ...
    for (const zAccount of zernioAccounts) {
      ...
      if (!zid) {
        console.warn("Skipping account witn no ID:", zAccount);
        continue;
      }
      ...
      if (!normalizedPlatform) {
        console.log(`Skipping unsapported platform: "${rawPlatform}"`);
        continue;
      }
